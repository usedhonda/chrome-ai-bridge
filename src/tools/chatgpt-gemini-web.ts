...
import type {AIResult} from './ai-helpers.js';
import {askAI, connectAI} from './ai-helpers.js';
...
function formatDebugInfo(provider: string, debug: ChatDebugInfo): string {
  ...
  debug.dom.markdowns.forEach((md, i) => {
    lines.push(
      `[${i}] class="${md.className}"${md.isResultThinking ? ' thinking=true' : ''}`,
    );
    lines.push(
  ...
  lines.push(
    debug.dom.lastArticleHtml.slice(0, 1500) +
      (debug.dom.lastArticleHtml.length > 1500 ? '...' : ''),
  );
  lines.push('');
  lines.push('innerText:');
  lines.push(
    debug.dom.lastArticleInnerText.slice(0, 800) +
      (debug.dom.lastArticleInnerText.length > 800 ? '...' : ''),
  );
  lines.push('');
...
export const askChatGptGeminiWeb = defineTool({
  ...
  handler: async (request, response) => {
    ...
    console.error(
      '[ask_chatgpt_gemini_web] Establishing connections in parallel...',
    );
    ...
    console.error(
      `[ask_chatgpt_gemini_web] Connections completed in ${connectionTime}ms`,
    );

    const chatgptOk =
      chatgptConn.status === 'fulfilled' && chatgptConn.value.success;
    const geminiOk =
      geminiConn.status === 'fulfilled' && geminiConn.value.success;

    console.error(
      `[ask_chatgpt_gemini_web] Connection status: ChatGPT=${chatgptOk}, Gemini=${geminiOk}`,
    );

    // 2. 接続成功した方のみクエリを実行
    const queries: Array<Promise<AIResult>> = [];
    ...
    if (chatgptOk) {
      ...
      const error =
        chatgptConn.status === 'rejected'
          ? chatgptConn.reason instanceof Error
            ? chatgptConn.reason.message
            : String(chatgptConn.reason)
          : chatgptConn.value?.error || 'Unknown error';
      failedProviders.push(`ChatGPT: ❌ 接続失敗 - ${error}`);
      console.error(
        `[ask_chatgpt_gemini_web] ChatGPT connection failed: ${error}`,
      );
    }
    ...
    if (geminiOk) {
      ...
      const error =
        geminiConn.status === 'rejected'
          ? geminiConn.reason instanceof Error
            ? geminiConn.reason.message
            : String(geminiConn.reason)
          : geminiConn.value?.error || 'Unknown error';
      failedProviders.push(`Gemini: ❌ 接続失敗 - ${error}`);
      console.error(
        `[ask_chatgpt_gemini_web] Gemini connection failed: ${error}`,
      );
    }
    ...
    if (queries.length > 0) {
      console.error(
        `[ask_chatgpt_gemini_web] Sending questions to ${queries.length} provider(s)...`,
      );
      ...
      console.error(
        `[ask_chatgpt_gemini_web] Queries completed in ${queryTime}ms`,
      );
